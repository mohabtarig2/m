<template>
<div>
<h2 class="text-center">{{$t('stone')}}</h2>
<div class="links-requests p-descrip-box  mb-3">
<router-link :to="{name:'StoneTenders'}" class="text-dark mr-3" > {{$t('tenders')}}</router-link>
...
<div v-if="tenders.length>0">
 <div class="s-blog-sidebar mb-3" v-for="(tendercr,index) in tenders" :key="index">
         ...
         <router-link class="font-weight-bold ml-2 text-dark " :to="{name:'StoneDetails',params:{id:tendercr.id}}">{{tendercr.title.substr(0,90 )}}
...
methods: {
 ...
 axios.get('/AllMyStoneTender').then(response=>{
