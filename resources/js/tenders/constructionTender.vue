...
<conditions></conditions>
    ...
    <h2 class="text-center text-muted"> {{$t('consr_tender')}} </h2>
...
<form  class="c-form-inner">
   ...
   <!-- <label for="exampleFormControlInput1">Photo</label>
    ...
    <input class="form-control mb-2" type="text" :placeholder="$t('short_descripe')"  v-model="title"
...
<div class="form-group">
         ...
         <input class="form-control mb-4 mt-3" type="text" :placeholder="$t('Name_Super_eng')"
           v-model="company_name" :class="[{'is-invalid': errorFor('company_name')}]" dir="rtl">
</div>
    <div
          class="invalid-feedback"
...
<!-- <div class="form-group">
        <small class="text-muted"> Pdf , dwg (autocad file)</small>
             <label class="d-block ">
                 <span class="btn btn-light" @click="$refs.dra" v-if="drainage==null"><span class=" fa fa-paperclip" ></i></span>

                <span class="btn  btn-primary " @click="$refs.dra" v-else><span class="fa fa-paperclip" ></i></span>
...
<span class="fa fa-close" v-if="resultdrainageSize" @click="RemoveFile('drainage')"></i>

<span v-if="resultdrainageSize!=null" ><span class="fa fa-file" style="color:#ebeef2"  ></i> {{resultdrainageName}}</span>
...
<!-- <div class="form-group">
        <small class="text-muted"> Pdf , dwg (autocad file)</small>
             <label class="d-block ">
                 <span class="btn btn-light" @click="$refs.water" v-if="Water==null"><span class=" fa fa-paperclip" ></i></span>

                <span class="btn  btn-primary " @click="$refs.water" v-else><span class="fa fa-paperclip" ></i></span>
...
<span class="fa fa-close" v-if="resultWaterSize" @click="RemoveFile('Water')"></i>

<span v-if="resultWaterSize!=null" ><span class="fa fa-file" style="color:#ebeef2"  ></i> {{resultWaterName}}</span>
...
<div class="text-right" dir="rtl">
        ...
        || Specifications==null|| identity==null || map==null || company_name==null || notes==null ||
         emirates==null">{{$t('post_now')}} </button>

<!-- 
         <button @click.prevent="submit('draft')" class="btn theme-btn primary mt-3 font-weight-bold"
         :disabled="threeD==null || Structure==null ||Architect==null
           || Specifications==null|| identity==null || map==null || company_name==null || notes==null || emirates==null">نشر لاحقا </button> -->
</div>
             <StyleFile></StyleFile>
...
import StyleFile from './style.vue';
import AllUae from '../auth/AllUae.vue';

export default {
  components: { conditions ,ValidationErrors, StyleFile, AllUae},
...
const result3 = getFileExtension(this.Specifications.name);
             ...
             if( result3 !='xlsx'){
                   this.FileNotSpecificationsAllowd =`should Xlsx `;
                    return ;
                }
...
// passing the filename
  ...
  const config = {
                ...
                axios.post('ConstrFilES',data,config).then(res=>{
...
const result3 = getFileExtension(this.identity.name);
             ...
             if(!this.identity.type.match('application/pdf')){
...
const result3 = getFileExtension(this.map.name);
             ...
             if(!this.map.type.match('application/pdf')){
                   this.FileNotmapAllowd =`should Pdf  `;
                    return ;
                }
...
console.log(this.type_post);
    ...
    let emirates =  this.emirates_selected;
        ...
        data.append('services', this.services);
