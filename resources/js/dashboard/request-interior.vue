...
<h2 class="text-center">{{$t('Flow_Inteior_tender')}}</h2>
<div class="links-requests mb-3">
<router-link :to="{name:'StoneTenders'}" class="theme-color mr-3" > {{$t('Interior_Tender')}}</router-link>
...
<a @click="requestdirect(request.type,request.id)">
           ...
           {{request.interior.title}}</div>





           <p class=" mb-2 " >{{request.offer_interior.textOffer.substr(0,80)}}.... </p>
...
methods:{
    ...
    getRequestConfirmed(){
        if(this.IsUser==10){
        axios.get('request/interior').then(res=>{
            this.data = res.data;
        })
        }
         if(this.IsUser==5){
        axios.get('request/InteriorCompany').then(res=>{
            this.data = res.data;
        })
        }
    },
